...
class CarDeterm(ControlAffineSystem):
    DIM_X = 6
    DIM_STATES = 4
    DIM_U = 2
    ...
    U_MIN = torch.tensor([-3., -3.]).reshape(1, -1, 1)
    U_MAX = torch.tensor([3., 3.]).reshape(1, -1, 1)
    ...
    def a_func(self, x: torch.Tensor) -> torch.Tensor:
        ...
        a = torch.zeros(bs, CarDeterm.DIM_X, 1).type(x.type())
        a[:, 0, 0] = x[:, 3, 0] * torch.cos(x[:, 2, 0])
        a[:, 1, 0] = x[:, 3, 0] * torch.sin(x[:, 2, 0])
        if CarDeterm.state_dist is not None:
            a[:, CarDeterm.state_dist, 0] += x[:, 4, 0]
        if CarDeterm.state_dist2 is not None:
            a[:, CarDeterm.state_dist2, 0] += x[:, 5, 0]
        return a

    def dadx_func(self, x: torch.Tensor) -> torch.Tensor:
        bs = x.shape[0]
        #x, y, theta, v, d = [x[:, i, 0] for i in range(Car.DIM_X)]
        dadx = torch.zeros(bs, CarDeterm.DIM_X, CarDeterm.DIM_X).type(x.type())
        ...
        if CarDeterm.state_dist is not None:
            dadx[:, CarDeterm.state_dist, 4] = 1
        if CarDeterm.state_dist2 is not None:
            dadx[:, CarDeterm.state_dist2, 5] = 1
        return dadx
    ...
    def dbdx_func(self, x: torch.Tensor) -> torch.Tensor:
        bs = x.shape[0]
        dbdx = torch.zeros(bs, CarDeterm.DIM_X, CarDeterm.DIM_U, CarDeterm.DIM_X).type(x.type())
        return dbdx
