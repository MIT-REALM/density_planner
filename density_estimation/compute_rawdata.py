...
def compute_data(iteration_number, samples_x, system, args, samples_t=None, save=True, plot=True):
    ...
    for j in range(100):
        ...
        if save:
            ...
            data_name = path + datetime.now().strftime("%Y-%m-%d-%H-%M-%S") +"_rawData_" + system.systemname +"_iter%d_xSamples%d_tSamples%d" % (iteration_number, samples_x, samples_t) + ".pickle"
